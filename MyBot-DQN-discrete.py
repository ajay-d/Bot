...
class Agent:
    def __init__(self, state_size, action_size):
        ...
        if self.load_model:
            self.model.load_weights("./model_%d/actor.h5" % (game_map.my_id))
            self.target_model.load_weights("./model_%d/actor.h5" % (game_map.my_id))
    ...
    def append_sample(self, prior_state, cur_state, action, points):
        self.memory.append((prior_state, cur_state, action, points))
    ...
    def build_model(self):
        ...
        output = Dense(self.action_size, activation='linear', kernel_initializer='he_uniform')(h3)

        dqn = Model(inputs=state_input, outputs=output)
        adam = Adam(lr=0.01)
...
while True:
    ...
    for ship in game_map.get_me().all_ships():
        ...
        #output is rewards length 28
        if 'MyAgent' not in locals():
            MyAgent = Agent(td.size, PLANET_MAX_NUM)
